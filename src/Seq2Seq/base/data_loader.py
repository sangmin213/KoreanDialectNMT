...
class Vocab:
    def __init__(self, sentences, input_level, device):
        ...
        self.s_freq = {'<unk>':0, '<pad>': 0, '<sos>': 0, '<eos>':0}
...
class Loader:
    ...
    def readJson(self, path, dia2std=True, copy=False):
        with open(path, 'r', encoding='utf8') as f:
            data = json.load(f)

        if dia2std:
            self.pairs = [[self._normalize(data['dialect'][str(idx)]), self._normalize(data['standard'][str(idx)])] 
                                for idx,_ in enumerate(data['standard'])]
        else:
            self.pairs = [[self._normalize(data['standard'][str(idx)]), self._normalize(data['dialect'][str(idx)])] 
                                for idx,_ in enumerate(data['standard'])]

        # if copy:
        #     total_data = data['standard'] + data['dialect']
        #     print(total_data[:10])
        #     exit(1)
        #     self.pairs = [[self._normalize(data['standard'][str(idx)]), self._normalize(data['dialect'][str(idx)])] 
        #                         for idx,_ in enumerate(data['standard'])]
