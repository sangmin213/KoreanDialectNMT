...
def train(model, iterator, optimizer, criterion, clip):
    ...
    for i, batch in enumerate(iterator):
        ...
        # for j in range(3): # >> 한 문장에 대해서 3번씩 확인하는 recursive
        src = batch[0]
        trg = batch[1]
        ...
        #     print()
        #     print(trg[:5])
        #     tmp = [int(torch.argmax(output[i])) for i in range(5)]
        #     print(tmp)
...
def train_model(model, model_type, train_iterator, valid_iterator, optimizer, criterion, CLIP, N_EPOCHS, model_pt_path):
    ...
    for epoch in range(N_EPOCHS):
        ...
        valid_loss = evaluate(model, model_type, valid_iterator, criterion)
