...
def clean_text(text):
    ...
    Returns:
        normalized sentence
    """
    text = re.sub('[-=+,#/\?:^$.@*~&%ㆍ!\(\)\[\]\<\>]', '', str(text))
    return text


def convert_to_dataset(data, std, dia):
    """
    ...
    Args:
        ...
        std: torchtext Field containing Standard sentence
        dia: torchtext Field containing Dialect sentence

    Returns:
        (Dataset) torchtext Dataset containing 'std' and 'dia' Fields
    """
    # drop missing values not containing str value from DataFrame
    missing_rows = [idx for idx, row in data.iterrows() if type(row.standard_form) != str or type(row.dialect_form) != str]
    ...
    list_of_examples = [Example.fromlist(row.apply(lambda x: clean_text(x)).tolist(),
                                         fields=[('std', std), ('dia', dia)]) for _, row in data.iterrows()]

    # construct torchtext 'Dataset' using torchtext 'Example' list
    dataset = Dataset(examples=list_of_examples, fields=[('std', std), ('dia', dia)])
...
def make_iter(batch_size, mode, train_data=None, valid_data=None, test_data=None):
    ...
    file_std = open('pickles/std.pickle', 'rb')
    std = pickle.load(file_std)

    file_dia = open('pickles/dia.pickle', 'rb')
    dia = pickle.load(file_dia)
    ...
    if mode == 'train':
        ...
        valid_data = convert_to_dataset(valid_data.loc[:,['standard_form','dialect_form']], std, dia)
        ...
        train_iter, valid_iter = ttd.BucketIterator.splits(
            ...
            sort_key=lambda sent: len(sent.std), # sent = sentence. 문장 길이를 기준으로 정렬
        ...
        test_data = convert_to_dataset(test_data.loc[:,['standard_form','dialect_form']], std, dia)
        ...
        test_iter, _ = ttd.BucketIterator.splits(
            (test_data, dummy),
            sort_key=lambda sent: len(sent.std),
            sort_within_batch=True,
            batch_size=batch_size,
            device=device)
...
class Params:
    ...
    def load_vocab(self):
        ...
        pickle_std = open('pickles/std.pickle', 'rb')
        std = pickle.load(pickle_std)

        pickle_dia = open('pickles/dia.pickle', 'rb')
        ...
        params = {'input_dim': len(std.vocab), 'output_dim': len(dia.vocab),
                  'sos_idx': dia.vocab.stoi['<sos>'], 'eos_idx': dia.vocab.stoi['<eos>'],
                  'pad_idx': dia.vocab.stoi['<pad>'], 'device': device}
